...
import type { LocationEdge } from "../graphql/generatedType/AllMapLocationsTypes";
...
export interface FranchiseLocationWithDistance extends LocationEdge {
  distance: number;
}
...
const useProximitySearchGlobal = (
  franchiseLocations: LocationEdge[]
): UseProximitySearchResult => {
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [nearestLocations, setNearestLocations] = useState<
    FranchiseLocationWithDistance[]
  >([]);
    ...
    try {
      ...
      const distances = franchiseLocations.map((location) => {
        if (
          ...
          location.node.locationPurpose.locationLong !== null
        ) {
...
export default useProximitySearchGlobal;
